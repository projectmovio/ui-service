...
function createHistoryAnimeItem(anime) {
    ...
    resultHTML +='<button class="btn btn-sm btn-danger d-inline" onclick="showConfirmationModal(' + animeId + ')"><i class="fas fa-minus fa-xs"></i></button>'
...
function showConfirmationModal(animeId) {
    animeIdToRemove = animeId;

    $('#removalConfirmationModal').modal(show = true)
}

function removeAnimeWatchHistory() {
    removeAnime(animeIdToRemove);
    animeIdToRemove = "";
}